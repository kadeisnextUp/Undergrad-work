# -*- coding: utf-8 -*-
"""
Created on Sun Feb 23 11:12:18 2025
...
def has_prefix(s, prefix):
    ...
    """
    "*** YOUR CODE HERE ***"
    #every linked list has a prefix of empty
    if prefix is empty:
        return True
    elif s is empty and prefix is not empty:
        return False
    else:
        #if the begining of s is the same as prefix
...
def has_sublist(s, sublist):
    ...
    """
    "*** YOUR CODE HERE ***"
    "General fucntion of has_prefix"
    #every linked list is gonna have empty somewhere 
    if sublist is empty:
        return True
    elif s is empty and sublist is not empty:
        return False
    else:
        #if sublist is at the begining of s return true
        if has_prefix(s, sublist):
            return True
        #recusivley move through the rest of the linked list 
        else:
...
def has_kitty_gene(dna):
    ...
    """
    "*** YOUR CODE HERE ***"
    #specific verison of has_sublist
    gene = link('C', link('A', link('T',link('C',link('A',link('T',rest=empty))))))
...
def count_change(amount, denominations):
    ...
    if   amount == 0:
        return 1
    elif denominations is empty:
        return 0
    else:
        #check to see if current coin can be used
        temp = amount - first(denominations)
        if temp >= 0:
            count = count_change(temp, denominations)
        #if not move to next coin
        else:
            count = 0
        countOther = count_change(amount, rest(denominations))
